...
from commonlib.models import Event, EventCategory, Category
from django.forms.models import model_to_dict
import datetime
from commonlib.database_management.event import EventManagement
from commonlib.database_management.like import LikeManagement
from commonlib.database_management.user import UserManagement
from commonlib.database_management.comment import CommentManagement
from commonlib.database_management.participant import ParticipantManagement
import os
...
def list_event(request):
    ...
    if limit < 0 or limit > 50:
        return utils.response_400("Limit should be in range 0-50.")
    if page < 0:
        return utils.response_400("Page must be greater than 0.")
    ...
    if category_id is not None:
        return handle_filter_category(category_id, title, limit, page)
...
def handle_filter_category(category_id, title, limit, page):
    # Check category exists
    if Category.objects.filter(id=category_id).count() == 0:
        return utils.response_404('Category does not exist.')
    ...
    if title is not None:
        ...
        total_offset = limit * (page - 1)
        for event in events:
            if EventCategory.objects.filter(category_id=category_id, event_id=event.id).count() != 0:
                offset += 1
                if offset > total_offset:
                    response.append(event.as_dict())
                if offset >= total_offset + limit:
                    return utils.response_200({
                        'events': response,
                        'next': next,
                        'previous': previous,
    ...
    if title is None:
        objs = EventCategory.objects.filter(category_id=category_id)
        total = objs.count()
        paginator = Paginator(objs, limit)
        event_list = []
        try:
            for event_category in paginator.page(page):
                event = Event.objects.get(id=event_category.event_id)
                event_list.append(event.as_dict())
        except:
            pass
        return utils.response_200({
